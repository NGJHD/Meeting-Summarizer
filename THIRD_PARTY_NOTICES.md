...
The code in this repository is MIT (see `LICENSE`). The application does not *contain*
any of the components below — `DOWNLOAD_MODELS.bat` fetches them from their publishers
into `bin/`, `models/` and `runtime/`. Each keeps its own licence, and an assembled
folder is a collection of separately-licensed works rather than a single one.

Every version here is pinned in `DOWNLOAD_MODELS.bat`, so this list stays true until
somebody deliberately changes a pin.
...
| [Silero VAD v5.1.2](https://huggingface.co/ggml-org/whisper-vad) (ggml) | Speech detection | MIT |
| [pyannote segmentation-3.0](https://huggingface.co/pyannote/segmentation-3.0) (ONNX re-export) | Speaker segmentation | MIT |
| [NVIDIA TitaNet-Large](https://huggingface.co/nvidia/speakerverification_en_titanet_large) (ONNX re-export) | Speaker embedding | **CC-BY-4.0** |
